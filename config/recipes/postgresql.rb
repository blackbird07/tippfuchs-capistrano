...
namespace :postgresql do
  ...
  task :create_db_user, roles: :db, only: {primary: true} do
    execute %{psql -U postgres -tAc "SELECT 1 FROM pg_user WHERE usename='#{postgresql_user}'" | grep -q 1 || psql -U postgres -c "create user #{postgresql_user} with password '#{postgresql_password}';"}
  end
  
  desc "Create a database for this application."
  task :create_database, roles: :db, only: {primary: true} do
    execute %{psql -U postgres -tAc "SELECT 1 FROM pg_database WHERE datname='#{postgresql_database}'" | grep -q 1 || psql -U postgres -c "create database #{postgresql_database} owner #{postgresql_user};"}
  end
end